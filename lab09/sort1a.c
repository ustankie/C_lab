...
void print_art(Food *p, int n, char *art)
{
    for (int i = 0; i < n; i++)
    {
        ...
        t.tm_sec = 0;
        t.tm_min = 0;
        t.tm_mday = new->valid_date.day;
        t.tm_mon = new->valid_date.month - 1;
        t.tm_year = new->valid_date.year - 1900;
        t.tm_isdst = -1;
        time_t td = mktime(&t);
        struct tm tl = *localtime(&td);
        // time_t t=(new->valid_date.day,new->valid_date.month,new->valid_date.year);
        int a = (strcmp(new->name, art));

        if (a == 0)
        {
            strftime(buff, 11, "%d.%m.%Y", &tl);
            printf("%.2f %d %s\n", new->price, new->amount, buff);
        }
    }
}
...
//     int days_in_m[]={0,31,28,31,30,31,30,31,31,30,31,30,31};
//     //int days_in_m2[]={31,29,31,30,31,30,31,31,30,31,30,31};
//     if((date->year % 4 == 0 && date->year % 100 != 0) || date->year % 400 == 0){
//         days_in_m[2]=29;
//     }
//     if((date->day)+days>days_in_m[date->month]){
//         if((date->month)==12){
//             date->year+=1;
//             date->month=1;
//         }
...
//             (date->month)++;
//         }
//         date->day=days_in_m[date->month](date->day)
...
{ // print_tab(food_tab,n);
    ...
    Food *addr = (Food *)bsearch(&date, food_tab, n, sizeof(Food), cmp_date);
    // printf("%d\n",result);
    // printf(" %s %lf %d %d.%d.%d\n", addr->name, addr->price, addr->amount, addr->valid_date.day, addr->valid_date.month, addr->valid_date.year);

    float sum = 0;
    Food *addr2 = addr;
    while (addr<(food_tab+n)&&((addr->valid_date).day == date.valid_date.day && (addr->valid_date).month == date.valid_date.month && (addr->valid_date).year == date.valid_date.year))
    {
        // printf(" %s %lf %d %d.%d.%d\n", addr->name, addr->price, addr->amount, addr->valid_date.day, addr->valid_date.month, addr->valid_date.year);
    ...
    addr = addr2;
    addr--;
    while (addr>=food_tab&&((addr->valid_date).day == date.valid_date.day && (addr->valid_date).month == date.valid_date.month && (addr->valid_date).year == date.valid_date.year))
    {
        // printf(" %s %lf %d %d.%d.%d\n", addr->name, addr->price, addr->amount, addr->valid_date.day, addr->valid_date.month, addr->valid_date.year);
