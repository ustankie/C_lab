...
void stream_to_list(List *p_list, FILE *stream, CompareDataFp cmp)
{
	...
	for (int j = 0; j < i; j++)
	{
		data = create_data_word(words[j], 0);
		if(cmp==NULL){
...
ListElement *find_insertion_point(const List *p_list, ListElement *p_element)
{
	...
	// printf("%d\n now: ",*(int *)p_element->data);
	// dump_int(ptr->data);printf("\n");
	if (p_list->compare_data(ptr->data, p_element->data) >= 0)
		return prev;
	return ptr;
}
