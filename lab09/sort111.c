...
void persons_shifts(Person *person_tab, int size, Parent *idx_tab, int no_parents)
{
    ...
    for(int i=0;i<no_parents;i++){
        ...
        //print_people(temp,children);
        memmove(person_tab+children_sum+children+1,person_tab+children_sum+1,(idx-children_sum-1)*sizeof(Person));
        memcpy(person_tab+children_sum+1,temp,children*sizeof(Person));
        

        
        for(int j=0;j<no_parents;j++){
            int a=idx_tab[j].index;
            if(a>(children_sum)&& a<(idx)){
                
                idx_tab[j].index+=children;
                //printf("%d\n",idx_tab[j].index);
            }
        }
        cperson->index=children_sum+1;
        if(i==0||i==1){
            printf("\n%d\n",cperson->index);
    ...
    //         {
    //             int j = 0;
    //             while (idx_tab[j].par_name != person_tab[i].parent)
    //             {
    //                 j++;
    //             }
    //             idx_tab[j].children -= 1;

    //             if (idx_tab[j].children == 0)
    //             {
    //                 memmove(idx_tab + j, idx_tab + j + 1, (no_parents - j - 1) * sizeof(Person));
    //                 // printf("%d\n", i);
    //                 no_parents--;
    //                 // idx_tab[j+1];
    //             }
