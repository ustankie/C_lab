...
ht_element *get_element(hash_table *p_table, data_union *data)
{
	...
	//printf("%s %d\n",((DataWord *)(data->ptr_data))->word,ptr==NULL);
	//printf("%d", p_table->compare_data(*data, ptr->data));
	while (ptr != NULL)
	{	
...
void insert_element(hash_table *p_table, data_union *data)
{
	...
	// data_union val=p_table->create_data((void*) data);
	// ht_element *prev=find_previous(p_table,val);
	size_t hash = p_table->hash_function(*data, p_table->size);
...
void remove_element(hash_table *p_table, data_union data)
{
	...
	// printf("a");
	if (p_table->compare_data(prev->data, data) == 0)
	{
...
data_union create_data_word(void *value)
{
	...
	// ((DataWord*)(b.ptr_data))->word=a;
	// ((DataWord*)(b.ptr_data))->counter=1;
	// printf("%s ",((DataWord*)(b.ptr_data))->word);
	// dump_word(*ptr);
	return *ptr;
}
...
void stream_to_ht(hash_table *p_table, FILE *stream)
{
	...
	while (fgets(buff, BUFFER_SIZE, stream))
	{
		...
		while (word != NULL)
		{
			...
			// data=malloc(sizeof(data_union));
			//  DataWord *a=malloc(sizeof(DataWord)*BUFFER_SIZE);
			//  a->word=tmp.ptr_data;
			//  a->counter=1;
			...
			//((data->ptr_data))=malloc(sizeof(DataWord));
			//((DataWord *)(data->ptr_data))->word=word;

			// data_union new_word=create_data_word((void *)word);
			// dump_word(new_word);
			//
			//  {
