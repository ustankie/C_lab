...
{ // printf("a\n");
	...
	// printf("%d \n",date1.day);
	//  int month1 = (((Food *)d1)->valid_date).month;
	//  int month2 = (((Food *)d2)->valid_date).month;
	//  int year1 = (((Food *)d1)->valid_date).year;
	//  int year2 = (((Food *)d2)->valid_date).year;
...
Food *add_record(Food *tab, int *np, ComparFp compar, Food *new)
{
	...
	{	
		Food *p;
		//addr+=1;
		// if(cmp((void *)new, (void *)addr)>0 && np>0){
		// 	addr+=1;
		// }
		...
		for (Food *i = tab+*np+1; i >(addr); i -= 1)
		{
			//printf("%s %s\n",i->name,new->name);
			//printf("%p\n",i);
			*i = *(i - 1);
