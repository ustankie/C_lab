...
double dbl_integr(Func2vFp f, double x1, double x2, int nx, double y1, double y2, int ny) {
	...
	double sum2=0;
	double h2;
	for(double h=x1;h<x2;h+=step1){

		for(double h2=y1;h2<y2;h2+=step2){
			sum1+=f(h,h2);
		}
	}
	sum1*=step1*step2;
...
double dbl_integr_normal_1(Func2vFp f, double x1, double x2, int nx, double hy, Func1vFp fg, Func1vFp fh) {
	...
	for(double i=x1;i<x2;i+=stepx){
		ny=ceil((fh(i)-fg(i))/hy);
		stepy=(fh(i)-fg(i))/ny;
		for(double j=fg(i);j<fh(i);j+=stepy){
			sum+=(f(i,j)*stepy);
		}
	}
...
double trpl_quad_rect(FuncNvFp f, double variable_lim[][2], const int tn[], BoundNvFp boundary) {
	...
	for(double x=(variable_lim[0][0]+stepx);x<(variable_lim[0][1]+stepx);x+=stepx){
		for(double y=(variable_lim[1][0]+stepy);y<(variable_lim[1][1]+stepy);y+=stepy){
			for(double z=(variable_lim[2][0]+stepz);z<(variable_lim[2][1]+stepz);z+=stepz){
				v[0]=x;
				v[1]=y;
				v[2]=z;
				if (boundary!=NULL && boundary(v,n)){ //czy tu powinno byc "==" czy "!=" ?
					sum+=func3v(v,n);
				}
