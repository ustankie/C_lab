...
double gauss(double A[][SIZE], const double b[], double x[], const int n, const double eps) {
	...
	while((is_not_diagonal_2(ZERO,n,indices))){
		while(ind>0 && ZERO[ind-1]==ZERO[ind]){
			//if(zero_on_diag(A,n))return NAN;
			if(abs(A[indices[ind]][ZERO[ind]])>abs(A[indices[ind-1]][ZERO[ind-1]])){
				multiplier=(-1)*A[indices[ind]][ZERO[ind]]/A[indices[ind-1]][ZERO[ind-1]];
				////printf("%lf\n",multiplier);
				multiply_row(A,n,indices[ind-1],multiplier);
                B[indices[ind-1]]*=multiplier;
				//print_mat(A,n,n);

				swap_rows_2(indices,n,ind,ind-1,ZERO);
				...
				multiplier=(-1)*A[indices[ind-1]][ZERO[ind-1]]/A[indices[ind]][ZERO[ind]];
				multiply_row(A,n,indices[ind],multiplier);
                B[indices[ind]]*=multiplier;
